 ...
 *
 * 三档驱动，自动探测，无需改代码：
 *  1. kv   —— 配了 Vercel KV / Upstash Redis 环境变量 → 写入 Redis（线上真实持久化）
 *  2. file —— 本地开发兜底，写入 .data/analytics.json（Vercel 上不持久，仅 dev 用）
 *  3. log  —— 都没有时，仅打函数日志（保持旧行为，绝不报错影响主流程）
 *
 * 合规：不读请求 IP、不存任何 PII；uv 用 HyperLogLog（KV）只存匿名 sessionId 的哈希基数。
 */
...
export function currentDriver(): 'kv' | 'file' | 'log' {
...
export async function resetFileData(): Promise<void> {
  if (currentDriver() === 'kv') throw new Error('KV 模式不支持后台清空');
